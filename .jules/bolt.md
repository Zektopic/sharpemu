...
**Action:** Always encapsulate cached arrays and their generation ID into an immutable class/record (`sealed record SnapshotCache(long Generation, VirtualMemoryRegion[] Regions)`) and store it as a `volatile` reference. This enables safe lock-free cache invalidation across threads without requiring expensive write locks for cache updates.
