...
internal static unsafe class VulkanVideoPresenter
{
    ...
    {
        // Performance rationale: Replacing LINQ .Where/.Select/.Concat/.Distinct chain with an inline stackalloc
        // buffer and manual uniqueness check strictly eliminates per-frame heap allocations and boxing overhead
        // from enumerators and HashSets, improving CPU performance and lowering GC pressure in the rendering hot path.
        Span<ulong> addresses = stackalloc ulong[128];
        var addressCount = 0;
        ...
        {
            _guestImageWorkSequences[addresses[index]] = sequence;
        }
    }
